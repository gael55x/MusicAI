...
def __parse_midi(data_fn):
    ...
    # Check if melody_stream has voices
    melody_voices = melody_stream.getElementsByClass(stream.Voice)
    if not melody_voices:
        # If no voices found, use the original melody stream
        melody_voice = melody_stream
    else:
        # Otherwise, compress voices into a single voice
        melody_voice = stream.Voice()
        for voice in melody_voices:
    ...
    comp_stream_parts = [j.flat for i, j in enumerate(midi_data) 
        if i in partIndices and isinstance(j, stream.Stream)]
    for part in comp_stream_parts:
        for element in part:
    ...
    for part in full_stream:
        ...
        if isinstance(part, stream.Stream):
            for element in part.getElementsByClass(instrument.Instrument):
                curr_part.insert(element.offset, element)
            for element in part.getElementsByClass(tempo.MetronomeMark):
                curr_part.insert(element.offset, element)
            for element in part.getElementsByClass(key.KeySignature):
                curr_part.insert(element.offset, element)
            for element in part.getElementsByClass(meter.TimeSignature):
                curr_part.insert(element.offset, element)
            for element in part.getElementsByOffset(476, 548, includeEndBoundary=True):
                curr_part.insert(element.offset, element)
        cp = curr_part.flat
